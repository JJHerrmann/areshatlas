...
CSV_PATH = OUTPUT_DIR / "drakharpan_deep_traversal_cells.csv"
META_PATH = OUTPUT_DIR / "drakharpan_deep_traversal_cells_metadata.json"
OVERLAY_PATH = REPO_ROOT / "public" / "maps" / "drakharpan-deep-traversal-cells-overlay.webp"

RNG_SEED = 84
SPACING = 32
JITTER = 9
SEA_LEVEL_M = -62.0
KM_PER_PIXEL_APPROX = 1.75
...
def load_surfaces() -> tuple[np.ndarray, dict[str, object]]:
    dem = np.array(Image.open(DEM_PATH), dtype=np.float32)
    meta = json.loads(CROP_META_PATH.read_text(encoding="utf8"))
    return dem, meta
...
def local_lonlat_from_xy(x: float, y: float, bounds: dict[str, float], width: int, height: int) -> tuple[float, float]:
...
def classify_profile(score: float, mean_slope_deg: float, ruggedness_m: float, coast_distance_km: float) -> str:
    if score >= 85 and ruggedness_m < 120 and mean_slope_deg < 4.0:
        return "open"
    if score >= 72:
        return "easy"
    if score >= 58:
    ...
    if score >= 26:
        return "severe"
    if coast_distance_km < 12 and mean_slope_deg > 14:
        return "cliffed_coast"
    return "extreme"


def main() -> None:
    dem, crop_meta = load_surfaces()
    height, width = dem.shape
    bounds = crop_meta["crop_bounds_aresh"]
    landmask = dem >= SEA_LEVEL_M

    smoothed_dem = ndi.gaussian_filter(dem, sigma=1.0, mode="nearest")
    gy, gx = np.gradient(smoothed_dem)
    slope = np.degrees(np.arctan(np.hypot(gx, gy) / 11.12))
    local_relief = ndi.maximum_filter(smoothed_dem, size=11) - ndi.minimum_filter(smoothed_dem, size=11)
    ...
    elev_penalty = normalize(np.maximum(mean_elev - SEA_LEVEL_M, 0.0), 0.0, 2600.0)
    slope_penalty = normalize(mean_slope, 0.0, 26.0)
    rugged_penalty = normalize(mean_relief, 0.0, 820.0)
    coast_bonus = 1.0 - normalize(mean_coast_dist, 0.0, 90.0)
    traversal_score = (
        100.0
        - slope_penalty * 43.0
        - rugged_penalty * 29.0
    ...
    with CSV_PATH.open("w", newline="", encoding="utf8") as handle:
        ...
        for cell_id, (seed_x, seed_y) in enumerate(points_xy):
            ...
            centroid_lat, centroid_lon = local_lonlat_from_xy(centroid_x, centroid_y, bounds, width, height)
            coast_distance_px_value = float(mean_coast_dist[cell_id])
            coast_distance_km = coast_distance_px_value * KM_PER_PIXEL_APPROX
    ...
    for cell_id in range(cell_count):
        profile = classify_profile(
            ...
            mean_slope_deg=float(mean_slope[cell_id]),
            ruggedness_m=float(mean_relief[cell_id]),
            coast_distance_km=float(mean_coast_dist[cell_id]) * KM_PER_PIXEL_APPROX,
        )
        profile_counts[profile] = profile_counts.get(profile, 0) + 1

    metadata = {
        "source_dem": str(DEM_PATH.relative_to(REPO_ROOT)),
        "source_crop_metadata": str(CROP_META_PATH.relative_to(REPO_ROOT)),
        ...
        "notes": [
            "Drakharpan-local refined Voronoi mesh generated directly from the normalized crop DEM.",
            "Cells are land-only against the -62 m shoreline baseline.",
            "This is the deeper local mesh and should be preferred over the centroid subset when editing regional traversal or later province/settlement assignments.",
        ],
        "outputs": {
