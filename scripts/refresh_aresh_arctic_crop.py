...
PREVIOUS_META_PATH = EXPORT_DIR / "aresh_arctic_16x9_metadata.json"
...
OUTPUT_METADATA_PATH = EXPORT_DIR / "aresh_arctic_16x9_metadata.json"
OUTPUT_DEM_PATH = EXPORT_DIR / "aresh_arctic_16x9_dem.tif"
OUTPUT_TOPO_PATH = EXPORT_DIR / "aresh_arctic_16x9_topobathy.png"
OUTPUT_TOPO_REF_PATH = EXPORT_DIR / "aresh_arctic_16x9_topobathy_current_coast_reference_cities.png"
OUTPUT_REPORT_PATH = EXPORT_DIR / "aresh_arctic_16x9_topobathy_current_coast_reference_cities_report.txt"
...
def archive_existing_exports() -> Path | None:
    existing = [item for item in EXPORT_DIR.iterdir() if item.is_file()]
    if not existing:
        return None
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    archive_dir = ARCHIVE_ROOT / f"aresh_arctic_azgaar_crop_{stamp}"
...
def main() -> None:
    ...
    archived_to = archive_existing_exports()
    ...
    report_lines = [
        "Fresh Aresh Arctic crop from current normalized world DEM",
        "========================================================",
        "",
        f"Archived previous bundle to: {archived_to.relative_to(REPO_ROOT) if archived_to else '[nothing archived]'}",
    ...
    fresh_metadata = {
        **previous_meta,
        ...
        "orientation_note": "Fresh crop regenerated from the current normalized rotated-world DEM stack.",
        "archived_previous_bundle_to": str(archived_to.relative_to(REPO_ROOT)) if archived_to else None,
        "outputs": {
            "dem": OUTPUT_DEM_PATH.name,
            "topobathy_png": OUTPUT_TOPO_PATH.name,
            "topobathy_current_coast_reference_cities_png": OUTPUT_TOPO_REF_PATH.name,
        },
    }
    OUTPUT_METADATA_PATH.write_text(json.dumps(fresh_metadata, indent=2), encoding="utf8")
    OUTPUT_REPORT_PATH.write_text("\n".join(report_lines) + "\n", encoding="utf8")

    print(OUTPUT_METADATA_PATH)
    print(OUTPUT_DEM_PATH)
    print(OUTPUT_TOPO_PATH)
